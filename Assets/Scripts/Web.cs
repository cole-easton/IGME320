...
namespace Spider
{
    ...
    public class Web : MonoBehaviour
    {
		...
		/// <summary>
		/// The combined length of all segments in the rope.
		/// </summary>
		...
		{
			get { return length; }
			set { length = Mathf.Abs(value); }
		}
        public Vector3 HeadPosition
		{
			get { return (!isAlive) ? Vector3.zero : head.point.position; }
			set { if (isAlive) head.point.position = value; }
		}
        public Vector3 TailPosition
		{
			get { return (!isAlive) ? Vector3.zero : tail.point.position; }
			set { if (isAlive) tail.point.position = value; }
		}
		public Vector3 HeadDirection { get { return (!isAlive) ? Vector3.zero : (head.segment.A.position - head.segment.B.position).normalized; } }
        public Vector3 TailDirection { get { return (!isAlive) ? Vector3.zero : (tail.segment.B.position - tail.segment.A.position).normalized; } }
        public Vector3 HeadVelocity
		{
			get { return (!isAlive) ? Vector3.zero : head.point.position - head.point.oldPosition; }
			set { if (isAlive) head.point.oldPosition = head.point.position - value; }
		}
        public Vector3 TailVelocity
		{
			get { return (!isAlive) ? Vector3.zero : tail.point.position - tail.point.oldPosition; }
			set { if (isAlive) tail.point.oldPosition = tail.point.position - value; }
		}
		public Vector3 HeadAttachmentPoint
		{
			get { return (!isAlive) ? Vector3.zero : head.attachmentPoint; }
			set { if (isAlive) head.attachmentPoint = value; }
		}
		public Vector3 TailAttachmentPoint
		{
			get { return (!isAlive) ? Vector3.zero : tail.attachmentPoint; }
			set { if (isAlive) tail.attachmentPoint = value; }
		}
		public bool IsHeadLocked
		{
			get { return (!isAlive) ? false : head.isLocked; }
			set { if (isAlive) head.isLocked = value; }
		}
        public bool IsTailLocked
		{
			get { return (!isAlive) ? false : tail.isLocked; }
			set { if (isAlive) tail.isLocked = value; }
		}
        public bool IsHeadAttached { get { return (!isAlive) ? false : head.attachment != null; } }
        public bool IsTailAttached { get { return (!isAlive) ? false : tail.attachment != null; } }
		#endregion
		...
		private class RopePoint
		{
			...
			public RopePoint() : this(Vector3.zero, Vector3.zero) { }
